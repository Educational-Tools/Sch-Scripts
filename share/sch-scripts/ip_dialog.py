...
MSG_ROUTE_CONFLICT_IP = 'Η διεύθυνση {0} χρησιμοποιείται ήδη σαν προεπιλεγμένη διαδρομή. Παρακάλω δώστε μια διαφορετική.'
...
MSG_SUGGEST_HOSTNAME = 'Προτείνεται να μετανομάσετε τον υπολογιστή σας σε <b>{0}</b>\n\n'
...
class IP4_Config(Network_Manager_DBus):
    ...
    def get_properties(self):
        ...
        try:
            ...
            ip, mask, route, dnss = (None, None, None, [None])
...
class Page:
    def __init__(self):
        ...
        self.builder.add_from_file('ui/ip_dialog.ui')
...
class Ip_Dialog:
    ...
    def watch_nm(self, interest_interfaces, prefered_hostname):
        ...
        if break_bool:
            p = common.run_command(['ltsp', 'dnsmasq'], True)
            while p.poll() is None:
                while Gtk.events_pending():
                    Gtk.main_iteration()
        ...
        for interface in interest_interfaces:
            ...
            if interface.page.method_entry.get_active() == 2 and \
                ...
                if test_ip != interface.existing_info.ip:
                    p = common.run_command(['arping', '-f', '-c1', '-w1', '-I', interface.interface, test_ip], True)
                    while p.poll() is None:
                        while Gtk.events_pending():
                            Gtk.main_iteration()
                    if p.returncode == 0:
        ...
        for interface in interest_interfaces:
            ...
            if interface.page.method_entry.get_active() == 0:
                ...
                if int32_to_string(ip).startswith('10.') and \
                    ...
                    # TODO: nic.sch.gr no longer resolves 10.x.y.z DNS queries
                    # Use something like `wget ts.sch.gr/repo/irc` that will see the external IP, if needed
                    found, hostname = (False, '')
                    if found:
